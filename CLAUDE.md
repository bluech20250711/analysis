...
**남은 구현**: `src/lib/hwpx/readingSection.ts`
...
- `buildHwpx.ts`의 `buildReadingSectionPoCHwpx(readingSectionXml)` — 원본 문서(표지+듣기 1-17번)는 그대로 두고 문서 맨 끝에 독해 문항을 순서대로 이어붙임(좌/우 배분 로직 없음)
- `scripts/hwpx-reading-poc.ts` (`npm run hwpx:reading-poc -- <경로>`) — 18번(표준형)+25번(도표, 이미지 placeholder)+27번(안내문, 이미지 placeholder) 3문항으로 검증(mimetype STORED, XML 유효성, `hp:tbl` 개수가 원본과 동일(3개)한지, 각주 개수가 17+3=20개인지, 신규/원본 내용 대조 확인). **40번, 41-42/43-45번은 코드로 구현·스모크 테스트까지 마쳤지만 이번 데모 파일에는 포함하지 않음**(별도 요청 시 데모 가능)
...
- **독해 25/27/28번 이미지 placeholder**: 실제로는 표가 아니라 이미지이지만 이미지 생성 파이프라인이 없어 내용과 무관한 기존 BinData 이미지를 자리표시용으로 재사용 (듣기 4번과 동일한 한계)
