...
- 🔧 **오디오 병합 — 신규 HTTP 500(`Internal Error`) 발견 → 수정, 재검증 필요**: TTS 클립 생성이 통과하면서 처음 도달한 병합(`merge-audio-background`) 단계에서 우리 코드가 만들지 않은 일반 에러(`Internal Error. ID: ...`)로 실패. 병합 요청 본문에 듣기 전체 분량의 TTS 오디오를 base64로 통째로 실어 보내고 있었는데, Background Function은 AWS Lambda 비동기 invoke 경로를 타 동기 invoke(6MB)보다 훨씬 작은 페이로드 한도를 가져(수 KB~1MB 수준) 이를 초과해 플랫폼 단에서 요청 자체가 거부된 것으로 보인다. 클립을 id로만 참조하는 경량 스펙(`MergeSegmentSpec`)으로 바꾸고 실제 오디오는 서버가 Netlify Blobs에서 직접 읽어오도록 구조를 변경. 아래 "오디오 병합 모듈" 절의 "병합 요청이 HTTP 500으로 거부됨" 참고. **실제 Netlify 배포에서 재검증 필요**
...
1. **오디오 병합 재테스트**: 경량 스펙(`MergeSegmentSpec`) + Blobs 직접 조회 방식으로 바꾼 뒤 실제로 병합이 500 없이 성공하는지 확인 — 성공하면 최종 mp3 다운로드까지 확인
...
**검증**: `npm run test:merge-audio`로 새 plan→resolve→merge 흐름이 여전히 정상 동작(세그먼트 14개, 실측 62.45초/예상 62초)함을 확인. **payload 크기 문제 자체는 이 세션에서 실제 대용량 오디오로 재현하지는 못했으나(TTS 키가 없어 짧은 사인파로만 테스트), AWS Lambda 비동기 invoke 페이로드 한도는 공식 문서로 확인된 사실이라 이 구조 변경이 근본 원인을 제거한다고 판단함 — 다음 재배포에서 실제 성공 여부 확인 필요**.
