...
- `src/lib/hwpx/readingStyleConfig.ts` — **스타일 값을 한 곳에 모은 설정 파일**. charPr/paraPr/borderFill ID(모두 header.xml에 이미 정의된 값 재사용), 2단 컬럼 폭/간격, placeholder 이미지 경로 등. **실제 독해 템플릿을 받으면 이 파일의 값만 교체하면 되도록 설계**
- `src/lib/hwpx/readingSection.ts`:
  ...
  - `renderTableReadingItem` — 25번(도표)·27-28번(안내문): 표준형 + `ReadingTableData({caption?, headers, rows})` 기반 HWP 표. 25/27/28을 하나의 렌더러로 공용 처리
  ...
  - `renderSharedPassageGroup` — 41-42/43-45(장문 공유 지문): 지문은 한 번만 넣고 하위 문항 여러 개를 이어붙임
...
- `scripts/hwpx-reading-poc.ts` (`npm run hwpx:reading-poc -- <경로>`) — 18번(표준형)+25번(도표형) 2문항으로 검증(mimetype STORED, XML 유효성, 신규/원본 내용 대조 확인). **40번, 41-42/43-45번, 이미지 placeholder 경로는 코드로 구현했지만 이번 데모에는 포함하지 않음**(별도 요청 시 데모 가능)
