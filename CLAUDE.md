...
- 🔧 **오디오 병합 내부 — 신규 "ffmpeg 바이너리 경로를 찾을 수 없습니다" 발견 → 수정, 재검증 필요**: payload 문제를 고친 뒤 병합 요청 자체는 통과했지만, `ffmpeg-static`이 배포 환경에서 실행 파일 경로를 못 찾는 문제 발견. `ffmpeg-static`은 바이너리를 `node_modules/ffmpeg-static/`(자기 자신의 `index.js` 바로 옆)에 설치하고 `__dirname` 기준 상대 경로로 그 위치를 계산하는데, Netlify의 esbuild 번들러가 이 패키지를 다른 코드처럼 인라인하려 하면서 바이너리 파일이 배포 패키지에서 누락되거나 상대 위치가 깨진 것으로 보인다. `netlify.toml`에 `external_node_modules = ["ffmpeg-static"]`를 추가해 이 패키지를 번들링 없이 원본 그대로 복사하도록 수정, 추가로 진단 로그도 강화. 아래 "오디오 병합 모듈" 절의 "ffmpeg 바이너리 경로를 찾을 수 없습니다" 참고. **이 세션엔 ffmpeg-static이 설치되지 않아(프록시 제약) 로컬 재현이 불가능했음 — 실제 Netlify 배포에서 재검증 필요**
...
1. **ffmpeg 재테스트**: `external_node_modules` 설정 후 "음성만 다시 생성" 버튼으로 병합이 실제로 끝까지 성공해 mp3가 다운로드되는지 확인. 안 되면 `merge-audio-background` Netlify Functions 로그에서 `[merge-audio-background] ffmpegPath=... exists=...` 진단 로그로 정확히 어느 단계가 실패하는지 확인
2. **PDF 재테스트**: `React is not defined` 수정 후 실제로 PDF가 502 없이 생성되는지 확인
3. **`ffmpeg-static` 설치 여부 확인**: Netlify 빌드 로그에서 `ffmpeg-static`의 postinstall이 성공하는지
4. **엔드투엔드 전체 성공 확인**: Gemini → TTS(폴링) → 병합(폴링) → HWPX/PDF 다운로드까지 브라우저에서 한 번 끝까지 성공하는 것 확인
5. **HWPX/PDF 결과물 육안 확인**: 실제 생성된 hwpx를 한글에서, pdf를 뷰어에서 열어 CLI로 검증한 내용(줄바꿈, 2단 배분, 각주, 이미지 placeholder 등)이 동일하게 보이는지
6. **HWPX 출력 파일명 ASCII 확인**: 다운로드 파일명이 한글 파일명 깨짐 없이 내려가는지(설계스펙 6절)
8. **문항 재사용 플로우 확인**: 페이지를 새로고침해도 마지막 생성된 문항이 유지되고, "○○만 다시 생성" 버튼으로 Gemini 재호출 없이 TTS/HWPX/PDF 각각을 재시도할 수 있는지(아래 "프론트엔드 통합 모듈" 절의 "문항 캐싱 + 단계별 재시도" 참고)
