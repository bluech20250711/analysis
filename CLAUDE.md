...
- 🔧 **TTS 생성 — `connectLambda` 수정 후에도 동일한 502(MissingBlobsEnvironmentError) 재현, 추가 수정**: HWPX/PDF는 통과했지만 TTS는 여전히 실패. Background Function에 Blobs 컨텍스트가 자동 주입되지 않는 근본 문제가 `connectLambda(event)` 한 번으로는 해결되지 않은 것으로 보인다(Netlify 공식 문서·포럼에도 Background Function에서 이 에러가 지속되는 사례가 다수 보고돼 있음 — 아래 "Netlify Blobs 모듈" 절 참고). **코드만으로 100% 보장할 수 없는 플랫폼 쪽 이슈일 가능성이 높다** — `event.blobs` 존재 여부를 확인하는 진단 로그를 추가했고, 그래도 실패하면 에러 메시지에 `NETLIFY_BLOBS_TOKEN` 등록 안내가 직접 뜨도록 수정. **다음 재배포에서 로그로 `event.blobs` 존재 여부를 확인하고, 계속 실패하면 Personal Access Token을 `NETLIFY_BLOBS_TOKEN`으로 등록하는 수동 설정이 사실상 필요할 가능성이 큼**
...
- ❓ **`merge-audio-background.ts` / `ffmpeg-static`**: TTS 자체가 아직 끝까지 성공한 적이 없어 그 다음 단계인 병합까지는 도달 못 해봄

### Phase 8 다음 확인 체크리스트

1. **TTS Blobs 진단 로그 확인**: 재배포 후 `generate-audio-background` Netlify Functions 로그에서 `[netlifyBlobsStore] event.blobs 존재 여부=...`를 확인 — `false`면 Background Function에는 애초에 blobs 컨텍스트가 실려오지 않는다는 뜻이므로 `NETLIFY_BLOBS_TOKEN` 수동 등록이 사실상 유일한 해결책
2. **`NETLIFY_BLOBS_TOKEN` 등록**: 1번에서 계속 실패하면 Netlify 대시보드 → User settings → Applications → New access token으로 Personal Access Token 발급 → 해당 사이트 Environment variables에 `NETLIFY_BLOBS_TOKEN`으로 등록 → 재배포
3. **PDF 재테스트**: `React is not defined` 수정 후 실제로 PDF가 502 없이 생성되는지 확인
4. **`ffmpeg-static` 설치 여부 확인**: Netlify 빌드 로그에서 `ffmpeg-static`의 postinstall이 성공하는지(TTS가 통과해야 이 단계까지 실제로 도달함)
