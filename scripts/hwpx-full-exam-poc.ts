...
// 실제 문항 품질이 아니라 "45문항 전체 조립 메커니즘"을 검증하기 위한 간단한 테스트 데이터
...
function standardReadingItem(number: number, type: string, instruction: string, imageRef?: string): ReadingItem {
  return {
    ...
    type,
    instruction,
    passage: `[PoC] ${number}번 지문. This is a short placeholder passage written only to verify the HWPX assembly pipeline for question ${number}. It intentionally repeats simple sentences so the line-wrap estimator has something realistic to measure against, without copying any real exam content.`,
    passageKo: `[PoC] ${number}번 지문의 한국어 해석입니다. 이 문장은 실제 시험 지문이 아니라 HWPX 조립 파이프라인 검증용 예시 텍스트입니다.`,
    imageRef,
    chartData: imageRef
...
const testReading: ReadingItem[] = [
  standardReadingItem(18, '목적 파악', '다음 글의 목적으로 가장 적절한 것은?'),
  standardReadingItem(19, '심경 변화', '다음 글에 드러난 心境 변화로 가장 적절한 것은?'),
...
const item40: ReadingItem = {
  number: 40,
  type: '요약문 완성',
  instruction: '다음 글의 내용을 한 문장으로 요약하고자 한다. 빈칸 (A), (B)에 들어갈 말로 가장 적절한 것은?',
  passage:
    '[PoC] 40번 지문. This is a short placeholder passage for the summary-completion question, written only to verify the HWPX assembly pipeline.',
...
const passage4142 =
  '[PoC] 41-42번 공유 지문. This longer placeholder passage is shared by two consecutive questions, written only to verify the HWPX shared-passage assembly path without copying any real exam content. It repeats simple explanatory sentences so the renderer has enough text to wrap across multiple lines.';
...
const passage4345 =
  '[PoC] 43-45번 공유 지문(서사문). This placeholder narrative passage is shared by three consecutive questions, written only to verify the HWPX shared-passage assembly path for the three-question long-passage group without copying any real exam content.';
...
const testReadingFull: ReadingItem[] = [...testReading, item40, item41, item42, item43, item44, item45];
...
async function main() {
  ...
  const buffer = await buildFullExamHwpx(testListening, testReadingFull);
